...
export type SectionOptionsComplete = SectionConfigurationRequired & SectionConfigurationOptional;

export class SectionComponent implements RenderableComponent {
  private options: SectionOptionsComplete;

  constructor(requiredOptions: SectionConfigurationRequired, optionalOptions: SectionConfigurationOptional = {}) {
    this.options = {
      ...requiredOptions,
      textFont: optionalOptions.textFont || Fonts.InterNormal,
  ...
  render(): Content[] {
    return [
      ...
      },
      {
        ...
        absolutePosition: {x: this.options.textPosition.x, y: this.options.textPosition.y},
